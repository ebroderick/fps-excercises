...
class Excercise0605TestSuite extends FunSuite with BeforeAndAfter with MockitoSugar {

   test("test double with map") {
     val mockRng = mock[RNG]
     when(mockRng.nextInt).thenReturn((-112412, mockRng))

     val (result1, rng1) = chapter06.Excercise0605.double(mockRng)
     assert(result1 > 0.0 && result1 < 1.0)

     when(mockRng.nextInt).thenReturn((-1, mockRng))

     val (result2, rng2) = chapter06.Excercise0605.double(mockRng)
     assert(result2 > 0.0 && result2 < 1.0)

     when(mockRng.nextInt).thenReturn((Int.MaxValue, mockRng))

     val (result3, rng3) = chapter06.Excercise0605.double(mockRng)
     assert(result3 > 0.0 && result3 < 1.0)

     when(mockRng.nextInt).thenReturn((Int.MinValue, mockRng))

     val (result4, rng4) = chapter06.Excercise0605.double(mockRng)
